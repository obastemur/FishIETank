...
function FpsMeter(initialScore, scoreLabel) {
    ...
    this.currentFrameRenderingStartTime = 0;
    this.currentFrameRenderingEndTime = 0;
    this.framesSinceLastCheckpoint = 0;
    this.frameRenderingTimeSinceLastCheckpoint = 0;

    //create canvas and divs
    // AZM: Benchmarkification
    //this.canvas = document.createElement("canvas");
    this.canvas = mock.fpsCanvas;
    ...
    // AZM: Benchmarkification
    this.fishCountSlot = document.getElementById("fishCountSlot");
    this.frameRateSlot = document.getElementById("frameRateSlot");
    this.frameRenderingTimeSlot = document.getElementById("frameRenderingTimeSlot");
...
// AZM: Benchmarkification
FpsMeter.prototype.BeforeRenderFrame = function() {
    this.currentFrameRenderingStartTime = new Date();
}

FpsMeter.prototype.AfterRenderFrame = function() {
    this.currentFrameRenderingEndTime = new Date();
    this.framesSinceLastCheckpoint++;
    this.frameRenderingTimeSinceLastCheckpoint += this.currentFrameRenderingEndTime - this.currentFrameRenderingStartTime;
}
...
FpsMeter.prototype.Draw = function (score, snap) {
    ...
    if (now.getSeconds() == this.currentSecond) {
        ...
        if (timingDelayReached || fpsNotChangedYet) {
            ...
            this.fishCountSlot.innerHTML = score;
            this.frameRateSlot.innerHTML = this.fps;
            if (this.framesSinceLastCheckpoint > 0) {
                this.frameRenderingTimeSlot.innerHTML = Math.round(100 * this.frameRenderingTimeSinceLastCheckpoint / this.framesSinceLastCheckpoint) / 100;
            } else {
                this.frameRenderingTimeSlot.innerHTML = "N/A"
            }
            this.framesSinceLastCheckpoint = 0;
            this.frameRenderingTimeSinceLastCheckpoint = 0;
        }
